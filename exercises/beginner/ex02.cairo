...
func light_star{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    # Make sure this amount is at least equal to `star.size`
    assert_le(_star.size, dust_owned)
    # Get the caller next available `slot`
    let (avail_slot) = slot.read(address)
    # Update the amount of dust owned by the caller
    dust.write(address, dust_owned - _star.size)
    # Register the newly created star, with a size equal to `star.size`
    star.write(address, avail_slot, _star)
    ...
    a_star_is_born.emit(account=address, slot=avail_slot, size=_star)
    return ()
end
